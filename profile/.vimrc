...
function! CBuild(action)
    if expand("%:e") == 'c'
        write
        let l:cmd = "!clear;"
        let l:cmd .= "mkdir debug 2> /dev/null; gcc -Og -Wall -Werror -g %:p -o debug/%:r;"
        let l:cmd .= "mkdir release 2> /dev/null; gcc -Wall -Werror %:p -o release/%:r;"
        if a:action == "debug"
            let l:cmd .= "gdb -q debug/%:r;"
        elseif a:action == "release"
            let l:cmd .= "release/%:r;"
        endif
        execute l:cmd
    endif
endfunction
map  <F4> :call CBuild("release")<CR>
imap <F4> <Esc> :call CBuild("release")<CR>
vmap <F4> <Esc> :call CBuild("release")<CR>
map  <leader><F4> :call CBuild("debug")<CR>
imap <leader><F4> <Esc> :call CBuild("debug")<CR>
vmap <leader><F4> <Esc> :call CBuild("debug")<CR>
